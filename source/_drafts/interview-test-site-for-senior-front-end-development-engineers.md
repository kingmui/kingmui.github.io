...
将一段文本先用 Hash 函数生成**消息摘要**，然后用发送者的**私钥加密**生成数字签名，与原文文一起传送给接收者。接下来就是接收者校验数字签名的流程了。
...
### [CommonJS 模块和 ES6 模块的区别](https://www.kingmui.cn/2020/08/04/the-difference-between-commonjs-mdule-and-es6-module/)

为什么 CommonJS 输出的是模块的拷贝？它是如何做到的？

### webpack 的打包流程？Loader 与 Plugin 的区别？如何做性能优化？

参考[Webpack漫谈](https://juejin.im/post/6867797346550284296)

### [Javascript 执行机制](https://www.kingmui.cn/2018/11/15/Javascript%E6%89%A7%E8%A1%8C%E6%9C%BA%E5%88%B6/)

### React 调用 setState 后发生了什么？函数式组件 Hooks 如何存储？

React 的事件机制是如何实现的？点击如下按钮控制台将输出什么？
...
const Demo: FC = () => {
  ...
  useEffect(() => {
    ...
    ref.current.addEventListener('click', fn, false);
    return () => {
      ref.current.removeEventListener('click', fn);
    }
  }, []);

  return <button ref={ref} onClick={handleClick}>点我</button>;
}
```
...
参考[JavaScript设计模式](https://www.kingmui.cn/2019/03/27/JavaScript%E8%AE%BE%E8%AE%A1%E6%A8%A1%E5%BC%8F/)

### 你知道哪些继承方式

参考[JavaScript深入之继承的多种方式和优缺点](https://github.com/mqyqingfeng/Blog/issues/16)
...
参考[Promise 的简易实现](https://github.com/kingmui/CodeDemo/blob/master/Promise/index.ts)

### 深拷贝的实现需要注意什么

参考[深拷贝的简易实现](https://github.com/kingmui/CodeDemo/blob/master/deepClone/index.ts)

### 垃圾回收机制

参考[深入理解 V8 的垃圾回收原理](https://www.kingmui.cn/2020/09/13/deeply-understand-the-principle-of-V8-garbage-collection/)

### 你了解的前端安全

参考[前端安全总结](https://www.kingmui.cn/2020/03/21/front-end-security-summary/)

### 函数式编程

参考[JavaScript 函数式编程](https://www.kingmui.cn/2019/06/02/JavaScript-Functional-Programming/)

### 什么是闭包

参考[学习Javascript闭包（Closure）](https://www.kingmui.cn/2018/05/05/learning_javascript_closures/)
...
本题考察 26 进制。`mui = 9 * 26 ** 0 + 21 * 26 ** 1 + 13 * 26 ** 2 = 9343`
...
### 你对我们公司是了解吗？用过我们什么产品？对我们的产品有什么建议吗

### 你面了哪些公司，选择一家公司最看重什么

### 在上家公司的绩效如何？Leader 都找你聊什么
