...
- 返回值：
  - 如果传入的参数是一个空的可迭代对象，则返回一个**已完成**（already resolved）状态的 `Promise`。<font color="red">当且仅当传入的可迭代对象为空时为同步</font>。
...
- 返回值：
  - 一个待定的 `Promise` 只要给定的迭代中的一个 `promise` 解决或拒绝，就采用第一个 `promise` 的值作为它的值，从而异步地解析或拒绝（一旦堆栈为空）。<font color="red">如果传的迭代是空的，则返回的 promise 将永远等待</font>。
